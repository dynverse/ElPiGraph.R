...
FindAssocited <- function(X, BootPG, TrimmingRadius = NULL) {
  ...
  if(is.null(TrimmingRadius)){
    AssocitedPoints <- sapply(BootPG, function(tStruct){
      PD <- PartitionData(X = X, NodePositions = tStruct$NodePositions, SquaredX = SquaredX, TrimmingRadius = tStruct$TrimmingRadius)
      return(PD$Partition != 0)
    })
  } else {
    AssocitedPoints <- sapply(BootPG, function(tStruct){
      PD <- PartitionData(X = X, NodePositions = tStruct$NodePositions, SquaredX = SquaredX, TrimmingRadius = TrimmingRadius)
      return(PD$Partition != 0)
    })
  }
